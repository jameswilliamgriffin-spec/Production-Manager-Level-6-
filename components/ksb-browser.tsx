...
import {
  assessmentOutcomes,
  assessmentOutcomesForPathway,
  byType,
  coreAssessmentOutcomes,
  coreKsbs,
  ksbsForPathway,
  pathways,
  typeLabels,
...
function aoMatches(ao: AssessmentOutcome, query: string) {
  if (!query) return true;
  const q = query.trim().toLowerCase();
  if (!q) return true;
  return (
    ao.id.toLowerCase().includes(q) ||
    ao.title.toLowerCase().includes(q) ||
    ao.text.toLowerCase().includes(q) ||
    ao.ksbs.some((code) => code.toLowerCase().includes(q))
  );
}

function AoItems({ list }: { list: AssessmentOutcome[] }) {
  return (
    <ol className="duty-list">
      {list.map((ao) => (
        <li key={ao.id} className="duty-card">
          <span className="duty-id">{ao.id} — {ao.title}</span>
          <p className="duty-text">{ao.text}</p>
          <ul className="duty-ksbs" aria-label={`KSBs covered by ${ao.id}`}>
            {ao.ksbs.map((code) => (
...
/**
 * Shared collapse/expand primitive for both the K/S/B groups and the AO groups.
...
function KsbGroups({ list, query, idPrefix }: { list: Ksb[]; query: string; idPrefix: string }) {
  ...
  if (!anyResults) {
    return <p className="ksb-empty">No KSBs here match “{query}”. Try a code such as K12, or a word like “budget”.</p>;
  }
...
export function KsbBrowser() {
  ...
  const [active, setActive] = useState<PathwayId>('screen-audio');
  ...
  // One entry per accordion in the Assessment Outcomes section: core, then each
  // pathway's own AOs in turn.
  const aoGroups = [
    { id: 'ao-core', label: 'Core Assessment Outcomes', list: coreAssessmentOutcomes },
    ...pathways.map((pathway) => ({
      id: `ao-${pathway.id}`,
      label: `${pathway.name} (${pathway.specialism}) Assessment Outcomes`,
      list: assessmentOutcomesForPathway(pathway.id),
    })),
  ];
  const anyAoResults = aoGroups.some((group) => group.list.some((ao) => aoMatches(ao, query)));
  const aoForceOpen = query.trim().length > 0;
    ...
    <>
      <div className="ksb-bar">
        <div className="ksb-bar-inner">
          <nav className="ksb-jump" aria-label="Sections on this page">
            ...
            <a href="#duties">Assessment Outcomes</a>
          </nav>
      ...
      <section id="core-ksbs" className="ksb-section" aria-labelledby="core-ksbs-heading">
        <ScrollReveal className="ksb-section-head">
          ...
          <p>
            These {coreKsbs.length} apply to you whichever option you take. They cover how a production is
            planned, resourced and run to schedule, the legal and regulatory ground every production manager
            has to know, and how you lead and work with the people around you.
          </p>
        </ScrollReveal>
      ...
      <section id="pathway-ksbs" className="ksb-section" aria-labelledby="pathway-ksbs-heading">
        <ScrollReveal className="ksb-section-head">
          <ScrambleText className="ksb-eyebrow" text="YOUR OPTION" />
          <h2 id="pathway-ksbs-heading">Option KSBs</h2>
          <p>
            These sit <strong>on top of</strong> the core above, and they are the ones that change with your
            option. Pick an option to see only what applies to it.
          </p>
        </ScrollReveal>

        <div className="ksb-tabs" role="tablist" aria-label="Choose an option">
      ...
      >
        <ScrollReveal className="ksb-section-head">
          <ScrambleText className="ksb-eyebrow" text="WHAT GETS ASSESSED" />
          <h2 id="duties-heading">Assessment Outcomes</h2>
          <p>
            ST1483 is assessed against {assessmentOutcomes.length} Assessment Outcomes rather than a
            weighted set of methods — {coreAssessmentOutcomes.length} core, plus {assessmentOutcomes.length - coreAssessmentOutcomes.length} split
            across the three options. Each one draws on a set of KSBs, listed against it, so you can see how
            the criteria above show up in what you actually have to demonstrate.
          </p>
        </ScrollReveal>
        ...
        {anyAoResults ? (
          <div className="ksb-groups">
            {aoGroups.map((group) => {
              const filtered = group.list.filter((ao) => aoMatches(ao, query));
              ...
              return (
                <Accordion
                  ...
                  forceOpen={aoForceOpen}
                  left={group.label}
                  count={query ? `${filtered.length} of ${group.list.length}` : group.list.length}
                >
                  <AoItems list={filtered} />
                </Accordion>
              );
            })}
          </div>
        ) : (
          <p className="ksb-empty">No Assessment Outcomes match “{query}”.</p>
        )}
      </section>
    </>
  );
